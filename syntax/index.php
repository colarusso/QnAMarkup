...
<p>
When run through <a href="../">an interpreter</a>, QnA-formatted text is transformed into an interactive HTML document. QnAs can be used as interactive decision trees or to build documents based on user input. Content behaviour is defined by placing text after one of ten tags. For example, a QnA interpreter will treat text following <code>Q:</code> tags as questions and text following <code>A:</code> tags as answers. Below you'll find a description of available tags and related resources. 
</p><p>
If you are new to QnA, <a href="../">play a little</a> before diving in below. Also, feel free to start with the <a href="#body">Body</a> section. That's where all the exciting stuff happens.
</p>
<h3>Sections</h3>
<ul>
...
<a name="header"></a>
<h2>Header</h2>
<p>The header is optional text appearing before the first non-header tag (those tags listed under <em>Body</em> below). The header can be empty or contain any of the following five header tags. The value of a tag is defined by the text between that tag and the next. The order of tags is unimportant as long as they appear before the first non-header tag. If you use a tag twice, the last value provided will be used to define a tag's value.</p>
...
<iframe src="<?php echo $home ?>i/?markup=Q%281%29%3A+first+question%0D%0AA%3A+first+answer+to+first+question%0D%0A%09Q%281.1%29%3A+first+question+under+the+first+answer+to+the+first+question%0D%0AA%3A+second+answer+to+first+question%0D%0A%09Q%281.2%29%3A+first+question+under+the+second+answer+to+the+first+question&font_family=Verdana%2C+Geneva%2C+sans-serif&font_size=14&line_height=20&col_width=500&frame_pad=15&radius=15&comp_bg=5489eb&comp_txt=ffffff&comp_link=e3fbfc&usr_bg=eeeeee&usr_txt=000000&usr_link=0000ff&sharing=2" style="width:300px;height:300px;border: solid 1px #555;"></iframe>
		...
		<p>After rendering, by <a href="../">an interpreter</a> every <code>Q</code> tag will have a unique <em>target ID</em> placed in a parenthetical between its Q and colon based on its relation to other <code>Q</code> tags. These ID values are updated with every rendering and can be used in conjunction with the <code>GOTO</code> tag to get users to <code>Q</code> tags that would otherwise be inaccessible to them given the linear nesting of questions and answers. 
...
<p>
<code>GOTO</code> calls can only appear at the end of a <code>Q</code> tag, and there can only be one <code>GOTO</code> call per tag.
</p>
...
<h2><code>A: <font color=black><em>some text/html</em></font></code>, 
	...
	<ul>		<p>
			...
			<p>
			The <code>A</code> tag must line up with the preceding <code>Q</code> tag (i.e., have the same number of tabs between it and the start of the line. As many <code>A</code> tags can follow a <code>Q</code> as you like. For example:
			</p>
...
A:
		...
		A:
			...
			<p>
			When used in conjunction with QnA's predefined Javascript functions (listed below), the <em>href</em> argument can be do some neat stuff above and beyond linking to things because you can uses a <em>href</em> to run Javascript. For example, you could use the <code>save2()</code> and <code>transcript()</code> functions to let users save their conversations.
			</p>
			<p><code</code></p>
...
<iframe src="<?php echo $home ?>i/?markup=Q%281%29%3A+Do+you+want+to+see+something+neat%3F%0D%0AA%3A+Yes.%0D%0A%09Q%281.1%29%3A+Cool.+Click+away.%0D%0A%09A%5Bjavascript%3Asave2%28%27transcript.txt%27%2C+transcript%28%29%29%3B%5D%3A+Save+conversation.%0D%0A%09%09Q%281.1.1%29%3AGOTO%3A1%0D%0AA%3A+No.%0D%0A%09Q%281.2%29%3AGOTO%3A1&font_family=Verdana%2C+Geneva%2C+sans-serif&font_size=14&line_height=20&col_width=500&frame_pad=15&radius=15&comp_bg=5489eb&comp_txt=ffffff&comp_link=e3fbfc&usr_bg=eeeeee&usr_txt=000000&usr_link=0000ff&sharing=2" style="width:300px;height:300px;border: solid 1px #555;"></iframe>
	...
	<p>
		If you enclose a variable name like so <code>&lt;x&gt;<font color=red><em>variable_name</em></font>&lt;/x&gt;</code> it will be replaced by that variable's value. For example, in the QnA below, if a user types in "David" the QnA's reply would read "Nice to meet you David."
	</p>
...
<iframe src="<?php echo $home ?>i/?markup=DOC%281%29%3ASHOPPING+LIST%0D%0A%0D%0AQ%281%29%3A+What+would+you+like+to+cook%3F%0D%0AA%3A+Garlic+Chicken%0D%0A%09DOC%281.1%29%3AGarlic+Chicken%0D%0A%094+boneless+skinless+chicken+breasts%0D%0A%094+garlic+cloves%2C+minced%0D%0A%094+tablespoons+brown+sugar%0D%0A%091+tablespoon+olive+oil%0D%0A%09additional+herbs+and+spices%2C+as+desired%0D%0A%09Q%281.1%29%3AGOTO%3A2%0D%0AA%3A+Mac+and+Cheese%0D%0A%09DOC%281.2%29%3AMac+and+Cheese%09%0D%0A%093%2F4+pound+dried+elbow+macaroni%0D%0A%091+1%2F2+cups+grated++sharp+cheddar+cheese%0D%0A%091%2F2+cup+grated+gruyere+cheese%0D%0A%091%2F3+cup+panko+bread+crumbs%2C+toasted+until+golden%0D%0A%09Q%281.2%29%3AGOTO%3A2%0D%0AQ%282%29%3A+Okay.+I+have+the+shopping+list+ready.+How+would+you+like+it%3F%0D%0AA%5Bjavascript%3Aalert%28doc%28%29%29%3B%5D%3A+In+an+alert+box.%0D%0A%09Q%282.1%29%3AGOTO%3A3%0D%0AA%5Bjavascript%3Asave2%28%27list.txt%27%2Cdoc%28%29%29%3B%5D%3A+As+a+file+I+can+save.%0D%0A%09Q%282.2%29%3AGOTO%3A3%0D%0AQ%283%29%3A+Enjoy+the+grub.&font_family=Verdana%2C+Geneva%2C+sans-serif&font_size=14&line_height=20&col_width=500&frame_pad=15&radius=15&comp_bg=5489eb&comp_txt=ffffff&comp_link=e3fbfc&usr_bg=eeeeee&usr_txt=000000&usr_link=0000ff&sharing=2" style="width:300px;height:475px;border: solid 1px #555;"></iframe>
	...
	<p>You can make use of the <code>submit2()</code> function (described below) to pass your document to either an HTML or Markdown parser/editor. So instead of saving a document directly to ones computer, a user could have a change to edit his/her document before saving.</p>
	</ul>

<a name="scripts"></a>
<h2>Predefined Javascript Functions</h2>
<p>All interactive QnA documents come preloaded with a set of five <a href="http://www.w3schools.com/js/js_functions.asp" target=_blank>Javascript functions</a>. As described above, these can be called from an <code>A</code> tag using the syntax: <code>A<font color=purple>[<font color=red>javascript:<em>function_name</em>();</font>]</font>:</code>. Below we'll walk through each of these functions, explaining what they do.</p>	
<h2><code>transcript(<font color=red><em>format</em></font>);</code></h2>
	<ul>
		<p>This function will return a transcript of the current QnA as it exists at the time it is called. For example, when selected, the following tag will display a transcript in an alert box. <code>A<font color=purple>[<font color=red>javascript:alert(transcript());</font>]</font>: <font color=black>button text</code></code></p>
		<p>
		This function accepts argument called <code><font color=red><em>format</em></font></code>. When set equal to <code>1</code>, the transcript will include HTML found in the original QnA questions and answers. Otherwise, all HTML will be removed from the presented transcript. For example, this call will include HTML <code>A<font color=purple>[<font color=red>javascript:alert(transcript('1'));</font>]</font>:</code>
...
<p>My hope is that this will grow over time as people point me to various parsers available around the web. The basic idea is that by using the <code>submit2()</code> function in conjunction with a document parser/editor, it is possible to run documents created within a QnA by a person for action, like tweaking, printing or saving. Imagine a QnA that used a combination of <code>DOC:</code> and <code>X:</code> tags to craft a custom document for a <em>pro se</em> litigant. At the end of the QnA, that document can be passed to a parser and the user can take some time to edit the text before printing or saving it. The thing is that the <code>DOC:</code> tag doesn't really care what format its content is in. It could be HTML, markdown, or LaTex. By passing that content to a parser, it can be rendered and placed in a form that's easier for a user to digest.</p>	
<h3>Local HTML/Markdown Parser &amp; Editor</h3>
 <ul> <p>For now, I've set up a local HTML and Markdown parser, available at: (1) <a href="<?php echo $home ?>doc/parse/html/" target=_blank><?php echo $home ?>doc/parse/html/</a></li>; or (2) <a href="<?php echo $home ?>doc/parse/markdown/" target=_blank><?php echo $home ?>doc/parse/markdown/</a> depending on your needs.</p>
 <p>It reads two variables which you can sent it using the <code>submit2()</code> function. The first <code>t</code> is the content to be parsed, and the second <code>i</code> is a set of instructions to be displayed after parsing. In fact, <code>submit2()</code> takes <code>i</code> in as an argument. That is <code>i</code> = <code><font color=red><em>instructions</em></font></code> as defined above. For example, the QnA below can be used to create a letter to Santa. Note: the document is in HTML. So you'll notice that line breaks are indicated by the HTML tag <a href="http://www.w3schools.com/tags/tag_br.asp" target=_blank>&lt;br&gt;</a>.
 </p>
...
X:<font color=red>whatiwant</font>
	DOC(2.1):	<font color=black>I would like </font>&lt;x&gt;<font color=black>whatiwant</font>&lt;/x&gt;<font color=black> for Christmas. I hope all is well with you up north.&lt;br&gt;&lt;br&gt;
			Sincerely,&lt;br&gt;
			</font>&lt;x&gt;<font color=black>myname</font>&lt;/x&gt;
